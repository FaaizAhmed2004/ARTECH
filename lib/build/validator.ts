...
import { getCurrentEnvironment, validateEnvironment } from './environment';
import { calculateOptimalMemorySettings } from './config';
 ...
 */
export function validateBuildConfiguration(): ValidationResult {
  ...
  try {
    ...
    result.errors.push('Unable to validate Next.js configuration: ' + (error as Error).message);
    result.valid = false;
  }
 ...
 */
export function generateBuildReport(): string {
